...
public class TestPlayer {
	...
	public static void main(String[] args) {
		...
		for(Player p : rcb2025) {
			System.out.println(p.getName() + " " + p.getRuns() + " runs");
		}
		
		for(Player p : rcb2025) {
			if(p.getRuns() > 2500)
			System.out.println(p.getName() + " has scored more than 2500 runs");
		}
		for(Player p : rcb2025) {
			if(p.getWickets() > 50)
			System.out.println(p.getName() + " has scored more than 50 wickets");
		}
		
		for(Player p : rcb2025) {
			if(p.getName().endsWith("l"))
			System.out.println(p.getName() + " " + p.getRuns() + " runs");
		}
